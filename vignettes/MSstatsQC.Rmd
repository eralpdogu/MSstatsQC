---
title: "MSstatsQC: An R/Bioconductor package for longitudinal system suitability monitoring and quality control for proteomic experiments"
author: 
...
#devtools::install_github("eralpdogu/MSstatsQC")
```
...
```{r, eval=TRUE, fig.width=8, fig.height=5}
# first 20 observations are used as a guide set. Only retention time and total area is used
...
# rf_model = MSstatsQC.ML.trainR(as.data.frame(guide.set), sim.size = 10)
# MSstatsQC.ML.deployR(Test.set, guide.set, rf_model=rf_model)
# 1. Önce H2O'yu başlat (Eğer zaten açıksa sorun çıkarmaz)
if (requireNamespace("h2o", quietly = TRUE)) {
  tryCatch({
    h2o::h2o.init(nthreads = -1)
  }, error = function(e) {
    warning("H2O başlatılamadı.")
  })
}

# 2. Veriyi KESİN olarak yükle (LazyData kapalı olduğu için bu şart!)
utils::data("S9Site54", package = "MSstatsQC", envir = environment())

# 3. Veriyi guide.set değişkenine ata ve data.frame olduğundan emin ol
S9Site54.dataML <- DataProcess(S9Site54[, -c(4, 5, 8)])
colnames(S9Site54.dataML)[1] <- c("idfile")
colnames(S9Site54.dataML)[2] <- c("peptide")
S9Site54.dataML$peptide <- as.factor(S9Site54.dataML$peptide)

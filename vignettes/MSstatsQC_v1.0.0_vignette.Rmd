...
This file can be also uploaded through Shiny interface. If the user prefers the web-interface then `Upload file` option should be used and a csv file which includes metrics and peptide level data should be uploaded. The recommended format includes `Acquired Time`, `Peptide name`, `Annotations` and data for any metrics such as `Retention Time`, `Total Peak Area` and `Mass Accuracy` etc. 
...
(d)-(f) `BestRetentionTime`, `TotalPeakArea`, `FWHM`, `MassAccuracy`, and `PeakAssymetry`, and other metrics: These columns define a *feature* of a peak for a specific peptide. 
...
* `precursor`: the name of precursor of interest.
...
* `ytitle`: the y-axis title of the plot. 
...
APlotFunction( S9Site54, precursor, L = 1, U = 5, metric, normalization = FALSE,
...
p<-APlotFunction(S9Site54, precursor="a peptide", metric="a metric")
...
APlotFunction(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime")
```
...
APlotFunction(data, "LVNELTEFAK", metric = "BestRetentionTime", selectMean = 28.5, selectSD = 1 )
```
...
```{r, eval=FALSE}
# Retention time >> mean is 28.5 and standard deviation is 1.0
# Peak assymetry >> mean is 1.0  and standard deviation is 0.01
ASummaryFunction(data, listMean=list("Retention time” = 28.5, “Peak asymmetry” = 1.0), listSD = list("Retention time” = 1, “Peak asymmetry” = 0.01))
```
...
APlotFunction(data, "peptide", metric = "metric", L=1, U=20) 
```
...
```{r, eval=FALSE}
# Retention time >> mean is 28.5 and standard deviation is 1.0
XmRChart(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime", type="mean", selectMean = 28.5, selectSD = 1)
```
...
XmRChart(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime", type="mean", L = 1, U = 20)
XmRChart(S9Site54, "LVNELTEFAK", metric = "TotalArea", type="mean", L = 1, U = 20)
```
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
XmRChart(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime", type="dispersion", L = 1, U = 20)
XmRChart(S9Site54, "LVNELTEFAK", metric = "TotalArea", type="dispersion", L = 1, U = 20)
```
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
CUSUMChart(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime", type="mean", L=1, U=20, ytitle="CUSUMm")
CUSUMChart(S9Site54, "LVNELTEFAK", metric = "TotalArea", type="mean", L=1, U=20, ytitle="CUSUMm")
CUSUMChart(S9Site54, "LVNELTEFAK", metric = "BestRetentionTime", type="dispersion", L=1, U=20, ytitle="CUSUMv")
CUSUMChart(S9Site54, "LVNELTEFAK", metric = "TotalArea", type="dispersion", L=1, U=20, ytitle="CUSUMv")
```
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
XmRChart(S9Site54, "TAAYVNAIEK", metric = "BestRetentionTime", type="mean", L = 1, U = 20)
ChangePointEstimator(S9Site54, "TAAYVNAIEK", metric = "BestRetentionTime", type="mean", L = 1, U = 20)
```
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
XmRChart(S9Site54, "YSTDVSVDEVK", metric = "BestRetentionTime", type="mean", L = 1, U = 20)
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
XmRRiverPlot(S9Site54, L=1, U=20)
XmRRadarPlot(S9Site54, L=1, U=20)
```
...
```{r, eval=TRUE}
# Retention time >> first 20 observations are used as a guide set
CUSUMRiverPlot(S9Site54, L=1, U=20)
CUSUMRadarPlot(S9Site54, L=1, U=20)
```
### Tip for Shiny users:
...
`DecisionMap()` functions is another summary function used in `MSstatsQC`. It is used to compare aggregated results over all analytes for a certain method such as XmR charts with the user defined criteria. Firstly, user defines the performance criteria and run `DecisionMap()` function to visualize overall performance. This function uses all the arguments of summary plots listed previously. Additionally, the following arguments are used

...
get_CP_tho.hat <- function(data, L, U, data.metrics, listMean, listSD) {
    ...
    for (metric in data.metrics) {
        for (j in seq_len(nlevels(data$Precursor))) {
            metricData <- getMetricData(data, precursors[j], L, U,
                metric = metric, normalization = TRUE,
                selectMean = listMean[[metric]], selectSD = listSD[[metric]]
            )
            mix <- rbind(
                data.frame(
                    tho.hat = CP.data.prepare(metricData, type = "mean")$tho.hat[1],
                    metric = metric, group = "Individual Value", y = 1.1
                ),
                data.frame(
                    tho.hat = CP.data.prepare(metricData, type = "variability")$tho.hat[1],
...
CUSUM.River.DataFrame <- function(data, data.metrics, L, U, listMean, listSD) {
    ...
    for (metric in data.metrics) {
        data.1 <- CUSUM.River.prepare(data,
            metric = metric, L, U, type = "mean",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]], decisionInterval = 5
        )
        data.2 <- CUSUM.River.prepare(data,
            metric = metric, L, U, type = "variability",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]], decisionInterval = 5
        )
...
XmR.River.prepare <- function(data, metric, L, U, type, selectMean, selectSD) {
    ...
    for (j in seq_len(length(precursors))) {
        metricData <- getMetricData(data, precursors[j],
            L = L, U = U, metric = metric,
            normalization = TRUE, selectMean, selectSD
        )
        counter[seq_along(metricData)] <- counter[seq_along(metricData)] + 1
...
XmR.River.DataFrame <- function(data, data.metrics, L, U, listMean, listSD) {
    ...
    for (metric in data.metrics) {
        data.1 <- XmR.River.prepare(data,
            metric = metric, L, U, type = "mean",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]]
        )
        data.2 <- XmR.River.prepare(data,
            metric = metric, L, U, type = "variability",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]]
        )
        data.2$pr.y <- -(data.2$pr.y)
...
XmR.Radar.Plot.prepare <- function(data, L, U, metric, type, group, XmR.type, selectMean, selectSD) {
    ...
    for (j in seq_len(length(precursors))) {
        ...
        plot.data <- XmR.data.prepare(metricData,
            L = L, U = U,
...
XmR.Radar.Plot.DataFrame <- function(data, data.metrics, L, U, listMean, listSD) {
    dat <- data.frame(
        peptides = c(), OutRangeQCno = c(), group = c(),
        orderby = c(), metric = c(), tool = c(),
        probability = c()
    )
    for (metric in data.metrics) {
        data.1 <- XmR.Radar.Plot.prepare(data, L, U,
            metric = metric,
            type = "mean", group = "Mean increase",
            XmR.type = "poz",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]]
        )
        data.2 <- XmR.Radar.Plot.prepare(data, L, U,
            metric = metric,
            type = "mean", group = "Mean decrease",
            XmR.type = "neg",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]]
        )
        data.3 <- XmR.Radar.Plot.prepare(data, L, U,
            metric = metric,
            type = "variability", group = "Variability increase",
            XmR.type = "poz",
            selectMean = listMean[[metric]], selectSD = listSD[[metric]]
        )
        data.4 <- XmR.Radar.Plot.prepare(data, L, U,
            metric = metric,
...
Decision.DataFrame.prepare <- function(data, metric, method, peptideThresholdRed,
    ...
    if (method == "XmR") {
        for (precursor in precursors) {
            metricData <- getMetricData(data, precursor,
                L = L, U = U,
                metric = metric, normalization = TRUE, selectMean, selectSD
            )
    ...
    plot.data <- data.frame(
        AcquiredTime = AcquiredTime[seq_len(max_QCno)],
...
SummaryPlot <- function(data = NULL, L = 1, U = 5, method = "CUSUM",
    ...
    gg <- gg + geom_smooth(method = "loess", aes(
        x = dat$QCno, y = dat$pr.y, colour = dat$group,
        group = dat$group
    ))
    gg <- gg + geom_point(data = tho.hat.df, aes(
        x = tho.hat.df$tho.hat, y = tho.hat.df$y,
        colour = "Change point"
    ))
    ...
    gg <- gg + guides(colour = guide_legend(override.aes = list(
...
sample_density <- function(guide.set, n) {
    ...
    for (k in 1:ncol(guide.set.scale)) {
        guide.set.scale[, k] <- bctrans(guide.set.scale[, k])
    }
...
add_features <- function(temp.Data) {
    for (i in 1:ncol(temp.Data)) {
        ...
        for (k in 2:length(temp.Data[, i])) {
            ...
            if (mean.increase[k] > 5) {
                mean.increase[k] <- max(0, (temp.Data[k, i] - d + 0))
            } else {
                mean.increase[k] <- max(0, (temp.Data[k, i] - d + mean.increase[k - 1]))
            }
            if (mean.decrease[k] > 5) {
                mean.decrease[k] <- max(0, (-d - temp.Data[k, i] + 0))
            } else {
                mean.decrease[k] <- max(0, (-d - temp.Data[k, i] + mean.decrease[k - 1]))
            }
            if (dispersion.increase[k] > 5) {
                dispersion.increase[k] <- max(0, (v[k] - d + 0))
            } else {
